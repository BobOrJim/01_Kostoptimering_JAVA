...
//hittar en optimal lösning, används när artificiella variabler används
public class Fas2A{
    int[] optimalBasLosning;
    double[][] mat;
    int[] basLosning;
    void inputFas2a(double[][] mat, int[] basLosning){
        this.mat = mat;
        this.basLosning = basLosning;
        System.out.println("FAS2A KÖRS");
        optimalBasLosning = new int [mat[0].length];//baslösningen är slut kordinaten, dvs antalet kolumner
...
//KOLLAR AVSLUTNINGS VILKOR & SÖKNING SAMT LAGRING/UTSKRIFT AV OPTIMAL BASLÖSNING
        if(in <= 0.001){
            ...
            int raknare = 0;
            System.out.println("Funktionen är nu maximerad...");
            for(int n=1; n<mat[0].length; n++){
                for(int m=0; m<basLosning.length; m++){
                    if(basLosning[m] == n && output != true){
                        ...
                        raknare++;
                        //System.out.print(mat[m+1][mat[0].length-1] +" ");
                        output = true;
                        //break;
                    }
                }
            ...
            //optimalBasLosning[optimalBasLosning.length-1] = (int) mat[0][mat[0].length-1];//lagrar optimerat värde
            System.out.print("Baslösningen är nu: ");
            printArray(basLosning);
            System.out.print("Baslösnings kordinaten är nu: ");
            printArray(optimalBasLosning);
            System.out.print("Matrisen är nu: ");
            printMat(mat);
            System.out.println("Fas2a IS NOW DONE!!");
            return;
            //System.exit(0);
        }
//BERÄKNAR UTVARIABEL
...
/*    void setOptimalBasLosning(int[] optimalBasLosning){
        this.optimalBasLosning = optimalBasLosning;
    }*/
